"""get_margin_short — cache-first margin/short fetcher.
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            cached = select_margin_short_rows(conn, symbol, wanted[0], wanted[-1])
            ...
            if missing:
                ...
                try:
                    raw = client.get_margin_purchase_short_sale(
                    ...
                    if raw:
                        insert_margin_short_rows(conn, raw, "finmind", fetched_at)

            final_rows = select_margin_short_rows(conn, symbol, wanted[0], wanted[-1])
        finally:
            if owns_conn:
                conn.close()

        missing_after_fetch = _missing_dates(wanted, [row["date"] for row in final_rows])
        ...
        if error_code is None:
            return _error_envelope(
                ...
                f"missing margin/short data for {symbol} in {wanted[0]}..{wanted[-1]}: "
