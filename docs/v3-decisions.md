...
| 16 | **採用 Mark Minervini SEPA 框架 + 保留 Volatility Targeting Sizing**（2026-04-28 加，cheatsheet bump v3.0→v3.1） | **採用**：SEPA 三柱（Trend Template 8 條 / Stage Analysis / VCP+Pivot Breakout）作為 Phase 2/3 選股 + 進場核心。**台股本地化**：RS Percentile 自建（IBD 式 252 日加權，門檻 65 vs 美股 70）、流動性以 5 日均成交額 ≥ NT$100M 替代 $ADV、停損保留 -6%（vs Minervini -7~8%，預留漲跌停緩衝）、避開季報 8/2 月前 10 個交易日。**不採用**：Minervini 1% Fixed-Risk Sizing — 沿用 §6.6 Volatility Targeting（Vol Targeting 在台股 ±10% 漲跌停下對極端高 ATR 股自帶保護，Fixed-Risk 在小型股會下太重）；Stage 4 個股不做空（Shioaji 模擬倉 + 平盤下放空法規限制）。詳 [`workflow-cheatsheet.md`](workflow-cheatsheet.md) v3.0→v3.1 Changelog、[`design-zh-TW.md`](design-zh-TW.md) §4.4.1.1 | 升級 v2 漏斗（量化初篩 → 籌碼 → 技術 → 基本面）為 SEPA 紀律框架；解了 v3.0 既有 RS 概念過弱、無 Stage Analysis、無完整 Trend Template 的缺口；既有 §6.6 Sizing 與 ATR 停損公式不動，CLAUDE.md §5 公式唯一權威表無變動 |
...
| 2026-04-28 | 決策 #16：採用 Mark Minervini SEPA 框架 + 保留 Vol Targeting Sizing；cheatsheet bump v3.0 → v3.1；同步更新 `llm-decision-schema.md`（+5 SEPA 欄位）/ `tools-contracts.md`（market_data_tool 加 get_rs_percentile/get_stage/get_trend_template；screener_tool 加 trend_template_filter/stage_filter；pattern_recognition_tool VCP 輸出擴充）/ `design-zh-TW.md` §4.4.1.1（台股本地化備註）|
