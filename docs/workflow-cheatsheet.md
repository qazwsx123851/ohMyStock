# 台股短線動能交易系統 — 各情境選股與交易邏輯速查 **v3.0**

> **版本**：v3.0 ｜ **更新日期**：2026-04-26 ｜ **前版**：v2.0（保留 git history）
> **用途**：四階段漏斗 + 盤中觸發 + 全域風險閘 + **LLM 閉環決策迴圈** 的混合工作流
> **策略定位**：短線動能交易（Swing Momentum），持有 **3-15 個交易日**（含 10 日時間停損）
> **決策優先序**：
...
- 股價 50-500 元
- 站上 20MA 且 20MA 向上
- 5 日均量 > 2,000 張 OR 均成交額 > 1 億
- 近 20 日漲幅前 20%

#### 第二層（產業熱度，計算 0-10 分，v2 由 20 分降為 10 分）
- 產業 5 日漲幅前 30% +4
- 八大行庫本週淨買超前 5 大產業 +3
- 外資+投信本週合計淨買超 +3

#### 第三層（籌碼確認 ~30-50 檔，v2 大幅擴充）
- 外資+投信 5 日合計淨買超 > 0
- **主力分點集中度 = (前 15 大買超分點淨買量) / (當日成交量) ≥ 25% 連 3 日**（v2 量化定義）
- **個股期未平倉量近 5 日增加**（v2 新增；法人佈局個股期常領先現股 1-2 日）
- 融資增幅 < 融券增幅
- 集保戶數遞減
- **借券餘額不在快速增加區（< +15% / 10 日）**（v2 新增）

#### 第四層（技術精選 + 輕基本面 + 情緒面，~10-15 檔）
- K 線型態：紅三兵 / 底部翻揚 / 整理突破 / **VCP / 杯柄 / 平台突破**（v2 擴充）
- 布林 Squeeze → Expansion
- 突破帶量 + 拉回縮量
- 近月營收 YoY > 平均、EPS 預估未下調
- **近 7 日法人調升目標價 ≥ 2 家 OR 重訊正面**（v2 新增）
...
### 逐檔重評分（**v2 新公式：35+45+10+10=100**）

```
═══════════════════════════════════════════════════════════
技術面（35）              ← v2 由 40 降為 35，解共線性
  趨勢結構（均線/EMA/布林整合）  12     ← 整併均線 8/站穩 5MA 4/EMA5>10/布林 6
  K 線品質（含 VCP/杯柄/旗形）   8      ← v2 新增動能聖杯型態
  RSI 位置（35-75 動能股放寬）   5
  RS 強弱                         5
  量價結構（含 OBV 累積）         5     ← v2 新增 OBV

籌碼面（45）              ← v2 由 40 升為 45
  外資 5 日淨買超                 8
  投信 5 日淨買超                 8
  主力分點集中度 ≥ 25%           10     ← v2 量化定義
  融資融券（融資減 + 股價漲）      5
  集保戶數遞減                    4
  借券餘額變化                   -5 ~ +5  ← v2 新增（高 = 負分）
  個股期未平倉量變化              5     ← v2 新增

產業熱度（10）            ← v2 由 20 降為 10
  熱門產業（5 日漲幅前 30%）      4
  八大行庫淨買超                  3
  雙法人加碼                      3
...
### 6.3 進場 Must-have（3 項全要，LLM 必須逐項驗證）

1. **站穩突破 / 收盤陽 K 確認**（突破當日收盤 > 開盤 > 突破點）
2. **量能 ≥ 1.5× 5 日均量**（縮量拉回模式則 < 0.6×）
3. **近 5 日 RS > 大盤**

### 6.4 加分項（8 項至少 4 項通過）

- 看漲 K 線型態（吞噬 / 晨星 / 紅 K 站上 20MA / **VCP / 杯柄突破**）
- 週線確認（站上週線 20MA OR 週線 MACD 未死叉）
- RSI 在 35-75（動能股上緣放寬）
- EMA5 > EMA10 > EMA20（多頭排列）
- 外資 OR 投信當日淨買超
- 主力分點集中度 ≥ 25% OR 集保戶數遞減
- 近 5 日融資增幅 < 10%
- 7 日內有正面催化劑（法人 / 重訊 / 營收）
...
### 動能聖杯（William O'Neil 系，最高優先）
- **VCP（Volatility Contraction Pattern）**：3-5 次回檔幅度遞減（15% → 10% → 6% → 3%），量能同步遞減 → 突破當天爆量
- **杯柄（Cup & Handle）**：U 型整理 7-65 週 + 5-25% 把手回檔
- **平台突破（Flat Base）**：5-7 週橫盤振幅 < 15% 後突破
...
實作參考：`pyti` + `stockstats` + 自寫 detector（`src/ohmystock/strategies/_kpattern/`）。
