...
定義對外資料 / 交易連線層的薄封裝：FinMind（基本面 / 籌碼面歷史資料）、Shioaji（永豐金證券模擬倉）、以及 CLI `smoke-test` 子命令把 FinMind / Shioaji / Anthropic 三方連線一次性驗證完。每個 client 為「最薄一層」連線層 — 只負責建連、認證、單一 happy-path 呼叫；fallback 鏈 / 快取 / retry / rate limit 等留給 Phase 1 的 `data_pipeline_skill` 與 Phase 2 的下單 / tick 訂閱實作。本 capability 是 Phase 0 三方連線冒煙測試的權威。
