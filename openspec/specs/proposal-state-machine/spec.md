# proposal-state-machine Specification

## Purpose

定義 `transition_proposal()` Python API 的契約 — 推進策略改動提案在 `pending → validating → {approved → {merged, rejected}, rejected}` 5 狀態圖上的合法 edge、自動把檔搬到對應 sink 子資料夾（`PENDING_REVIEW/` / `merged/` / `rejected/`）、append 變更紀錄、原子寫入。本 capability 不含 WFA 驗證引擎、admin endpoint、PR 自動化（皆為後續 change）。SSOT 對 `proposals/README.md` §2 + cheatsheet §16。
