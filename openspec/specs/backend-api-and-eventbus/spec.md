...
本 capability 階段 `/api/admin/events` SHALL 仍為**無認證**端點（Bearer auth 由後續 change `web-admin-bearer-auth` 在 Phase 4 加上）。`/api/public/events` 不在本 capability 範圍。Mask serializer 不在本 capability 範圍。
