...
定義 LLM API 成本追蹤元件：以 `@track_llm_cost` async decorator 包住所有 Anthropic API 呼叫，把 input/output token 與美金成本寫入 SQLite `llm_costs` 表，並提供月度成本聚合查詢。本 capability 是預算追蹤（USD ~$31–36/月）與 9 條安全防線中「LLM 預算熔斷」的資料源。計費表 hardcode 為模組頂層常數，避免從外部資源動態載入造成「靜默計算為 0」的失效模式。
