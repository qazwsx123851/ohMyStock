...
- [x] 5.4 ~~Run `python scripts/backfill_rs_rating.py --days 5` against the local DB once~~ — **WON'T-DO at archive time per user**: env-gated (live FinMind sponsored-member access + populated `bars_daily` cache); no-raise + floor-failure paths are unit-test-covered (Task 4.5), so the real run mainly surfaces local env state. Re-open as a follow-up when FinMind/`bars_daily` are warmed.
- [x] 5.5 Added CLAUDE.md §5 SSOT row pointing at `openspec/specs/rs-percentile/spec.md（archive 後）+ src/ohmystock/sepa/rs.py + src/ohmystock/sepa/rs_loader.py + src/ohmystock/data/disposition.py + scripts/backfill_rs_rating.py` — done at archive time per established repo pattern.
