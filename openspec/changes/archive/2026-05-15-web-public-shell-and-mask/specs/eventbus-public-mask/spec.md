...
The initial mapping SHALL cover all 16 event types listed in
`docs/backend-eventbus.md` §3.2 (`screener_started`, `screener_completed`,
`pattern_detected`, `decider_thinking`, `decision_made`, `awaiting_confirm`,
`order_sent`, `journal_written`, `journal_queried`, `review_node_started`,
`review_completed`, `proposal_created`, `wfa_started`, `wfa_passed`, `wfa_failed`,
`risk_off_triggered`) with the exact field sets specified in `docs/backend-eventbus.md`
§4.2.
...
For every event_type whose whitelist contains `masked_symbol`, if the input payload
contains a `symbol` key, the serializer SHALL:

1. Replace it with `masked_symbol = mask_table.mask(symbol)`.
2. If `industry_hint` is also whitelisted for that event type, set
   `industry_hint = mask_table.industry_of(symbol)`.

The original `symbol` key SHALL NOT appear in the output (enforced by DENYLIST).
...
For event types where `reasoning_summary` is whitelisted (currently `decision_made`),
if the input payload contains a `reasoning` string, the serializer SHALL produce
`reasoning_summary` by applying `re.sub(r"\b\d{4}\b", "STK-?", reasoning)`. The original
`reasoning` key SHALL be dropped (DENYLIST enforced).
