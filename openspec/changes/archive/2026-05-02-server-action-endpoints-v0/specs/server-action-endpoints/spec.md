...
本 capability 涵蓋的所有端點 SHALL **不**檢查 `Authorization` header；不帶 token 的 request SHALL **不**回 401 / 403。Bearer auth 將由後續 capability `web-admin-bearer-auth` 加上，本 capability 不混入。

#### Scenario: screener endpoint 不帶 Authorization 仍能運作
- **WHEN** 對 `POST /api/admin/screener/run` 發 valid body 但**不**帶 `Authorization` header
- **THEN** HTTP status 為 200（success path）或對應的非-401/403 status；不為 401、不為 403

#### Scenario: confirm endpoint 不帶 Authorization 仍能運作
- **WHEN** 對 `POST /api/admin/confirm-gate/confirm` 發 valid body 但**不**帶 `Authorization` header
- **THEN** HTTP status 不為 401、不為 403（依資源狀態回 200/404/409/502）
