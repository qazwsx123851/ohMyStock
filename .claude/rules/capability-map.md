...
| web-admin scenario-gaps（DB-B1 risk gate 三色燈 + DB-B2 月度熔斷 banner + DB-B3 成本進度條 + CG-B1 confirm override + CG-B2 reject reason + ST-B1 連線測試 + ST-B2 額度/模型分布 + ME-B1 memory write + PR-B1） | `archive/2026-06-07-web-admin-scenario-gaps` + `src/ohmystock/swarm/market_risk_gate.py` + `src/ohmystock/safety/sizing_clamp.py` + `src/ohmystock/api/routes/{stats,settings,confirm_gate}.py` + `web-admin/src/pages/{DashboardPage,SettingsPage,PaperOverviewPage,MemoryPage}.tsx` |
| Proposal manual verdict（D7：WFA 驗證後停在 validating，人工 Approve/Reject；`run_validation` 加 `auto_transition`，CLI 不變） | `archive/2026-06-12-proposal-manual-verdict` + `src/ohmystock/validation/wfa.py` + `src/ohmystock/api/routes/proposals.py` + `web-admin/src/components/{validation-dialog,transition-dialog}.tsx` |
